public class Main {
    ...
    Собрать графический интерфейс проекта мессенджера (скриншоты можно посмотреть в материалах к уроку)
    Отправлять сообщения из текстового поля сообщения в лог по нажатию кнопки
...
P.S. в презентации можно увидеть начало проекта с сервером, но я бы рекомендовал посмотреть уже после того,
